...
def sms_command(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ ভুল ফরম্যাট।\nসঠিক ফরম্যাট: `/sms <নম্বর> <মেসেজ>`"); return
    cursor.execute("SELECT sms_sent, last_sms_date, bonus_sms FROM users WHERE user_id = ?", (user_id,))
    user_data = cursor.fetchone(); today = str(datetime.date.today())
    if not user_data:
        cursor.execute("INSERT INTO users (user_id, last_sms_date) VALUES (?, ?)", (user_id, today)); conn.commit(); sms_sent, bonus_sms = 0, 0
    else:
        if user_data[1] != today:
            cursor.execute("UPDATE users SET sms_sent = 0, last_sms_date = ? WHERE user_id = ?", (today, user_id)); conn.commit(); sms_sent = 0
        else: sms_sent = user_data[0]
        bonus_sms = user_data[2]
    total_limit = 10 + bonus_sms
    if sms_sent >= total_limit:
        bot.reply_to(message, f"আপনি আপনার দৈনিক SMS পাঠানোর সীমা ({total_limit} টি) অতিক্রম করেছেন।"); return
    cursor.execute("SELECT COUNT(*) FROM sms_log WHERE user_id = ? AND phone_number = ? AND DATE(timestamp) = ?", (user_id, phone_number, today)); same_number_count = cursor.fetchone()[0]
    if same_number_count >= 4:
        bot.reply_to(message, "আপনি এই নম্বরে দিনে সর্বোচ্চ ৪টি SMS পাঠাতে পারবেন।"); return
    try:
        ...
        if response.status_code == 200:
            ...
            cursor.execute("INSERT INTO sms_log (user_id, phone_number, message, timestamp) VALUES (?, ?, ?, ?)", (user_id, phone_number, sms_text, datetime.datetime.now().isoformat())); conn.commit()
            bot.reply_to(message, f"✅ '{phone_number}' নম্বরে আপনার SMS সফলভাবে পাঠানোর জন্য অনুরোধ করা হয়েছে।")
        else: bot.reply_to(message, f"API থেকে সমস্যা হয়েছে। স্ট্যাটাস কোড: {response.status_code}")
    except requests.exceptions.RequestException: bot.reply_to(message, "API সার্ভারের সাথে সংযোগ করা যাচ্ছে না।")

@bot.message_handler(commands=['profile', 'balance'])
def profile_command(message):
    if not is_channel_member(message.from_user.id):
        bot.reply_to(message, "অনুগ্রহ করে প্রথমে চ্যানেলে যোগ দিন।", reply_markup=force_join_keyboard()); return
...
def show_profile_info(chat_id, user_id, message_id=None):
    cursor.execute("SELECT sms_sent, last_sms_date, bonus_sms FROM users WHERE user_id = ?", (user_id,)); user_data = cursor.fetchone()
    sms_sent_today = user_data[0] if user_data and user_data[1] == str(datetime.date.today()) else 0
    bonus_sms = user_data[2] if user_data else 0; daily_limit = 10; remaining_sms = (daily_limit - sms_sent_today) + bonus_sms
    cursor.execute("SELECT COUNT(*) FROM sms_log WHERE user_id = ?", (user_id,)); total_sent_ever = cursor.fetchone()[0]
    ...
    keyboard = types.InlineKeyboardMarkup(); keyboard.add(types.InlineKeyboardButton("🔄 রিফ্রেশ", callback_data="show_profile"))
    try:
        if message_id: bot.edit_message_text(profile_text, chat_id, message_id, parse_mode="Markdown", reply_markup=keyboard)
        else: bot.send_message(chat_id, profile_text, parse_mode="Markdown", reply_markup=keyboard)
    except telebot.apihelper.ApiTelegramException as e:
        if "message is not modified" not in str(e): raise e

@bot.message_handler(commands=['help'])
def help_command(message):
    help_text = "📜 **বট কমান্ড তালিকা**\n\n**সাধারণ কমান্ড:**\n`/start` - বট চালু করুন।\n`/sms` - SMS পাঠান।\n`/profile` - প্রোফাইল দেখুন।\n`/history` - লগ দেখুন।\n`/referral` - রেফারেল লিঙ্ক পান।\n`/resend` - শেষ মেসেজ আবার পাঠান।\n`/draft` - মেসেজ ড্রাফট করুন।\n`/drafts` - ড্রাফট দেখুন।\n\nℹ️ যেকোনো সমস্যায় অ্যাডমিনের সাথে যোগাযোগ করুন।"
...
def resend_command(message):
    ...
    if not is_channel_member(user_id):
        bot.reply_to(message, "অনুগ্রহ করে প্রথমে চ্যানেলে যোগ দিন।", reply_markup=force_join_keyboard()); return
    cursor.execute("SELECT phone_number, message FROM sms_log WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1", (user_id,)); last_sms = cursor.fetchone()
    if last_sms: message.text = f"/sms {last_sms[0]} {last_sms[1]}"; sms_command(message)
    else: bot.reply_to(message, "আপনার কোনো পুরনো মেসেজ পাওয়া যায়নি।")
...
def draft_command(message):
    ...
    try:
        _, phone_number, sms_text = message.text.split(maxsplit=2)
        cursor.execute("INSERT INTO drafts (user_id, phone_number, message, created_at) VALUES (?, ?, ?, ?)", (user_id, phone_number, sms_text, datetime.datetime.now().isoformat())); conn.commit()
        bot.reply_to(message, "✅ মেসেজটি সফলভাবে ড্রাফটে সেভ করা হয়েছে।")
    except ValueError: bot.reply_to(message, "❌ ভুল ফরম্যাট।\nসঠিক ফরম্যাট: `/draft <নম্বর> <মেসেজ>`")

# --- সাধারণ মেসেজ হ্যান্ডলার (অ্যাডমিন ইনপুটের জন্য) ---
@bot.message_handler(func=lambda message: True)
def handle_admin_input(message):
    user_id = message.from_user.id
    cursor.execute("SELECT temp_admin_action FROM users WHERE user_id = ?", (user_id,)); action_data = cursor.fetchone()
    if not action_data or not action_data[0]: return
    action_type = action_data[0]
    cursor.execute("UPDATE users SET temp_admin_action = NULL WHERE user_id = ?", (user_id,)); conn.commit()
    if action_type == "set_bonus":
        try:
            target_user_id, bonus_amount = map(int, message.text.split())
            cursor.execute("UPDATE users SET bonus_sms = bonus_sms + ? WHERE user_id = ?", (bonus_amount, target_user_id)); conn.commit()
        ...
        except (ValueError, IndexError): bot.send_message(message.chat.id, "❌ ভুল ফরম্যাট। আবার চেষ্টা করুন।")
    elif action_type == "get_user_sms":
        try:
            target_user_id = int(message.text)
            cursor.execute("SELECT COUNT(*) FROM sms_log WHERE user_id = ?", (target_user_id,)); total_sms_sent = cursor.fetchone()[0]
            cursor.execute("SELECT phone_number, timestamp FROM sms_log WHERE user_id = ? ORDER BY timestamp DESC LIMIT 20", (target_user_id,)); logs = cursor.fetchall()
            if not logs: bot.send_message(message.chat.id, f"ব্যবহারকারী {target_user_id} এর কোনো লগ নেই।"); return
            log_text = f"📜 **ব্যবহারকারী {target_user_id} এর SMS লগ:**\n\n📊 **মোট পাঠানো SMS:** {total_sms_sent} টি\n-------------------------------------\n\n"
            for log in logs:
                dt_obj = datetime.datetime.fromisoformat(log[1]); formatted_time = dt_obj.strftime('%Y-%m-%d %H:%M')
                log_text += f"📞 **নম্বর:** `{log[0]}`\n🗓️ **সময়:** {formatted_time}\n---\n"
            bot.send_message(message.chat.id, log_text, parse_mode="Markdown")
        except ValueError: bot.send_message(message.chat.id, "❌ ভুল ইউজার আইডি। শুধুমাত্র সংখ্যা ব্যবহার করুন।")

# --- বাটন ক্লিকের উত্তর ---
@bot.callback_query_handler(func=lambda call: True)
def handle_callback_query(call):
    user_id = call.from_user.id; action = call.data
    if not is_channel_member(user_id):
    ...
    if action == "main_menu":
        ...
        show_profile_info(call.message.chat.id, user_id, call.message.message_id); bot.answer_callback_query(call.id)
        ...
        page = int(action.split('_')[2]); per_page = 5; offset = (page - 1) * per_page
        cursor.execute("SELECT phone_number, timestamp FROM sms_log WHERE user_id = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?", (user_id, per_page, offset)); logs = cursor.fetchall()
        cursor.execute("SELECT COUNT(*) FROM sms_log WHERE user_id = ?", (user_id,)); total_logs = cursor.fetchone()[0]
        total_pages = (total_logs + per_page - 1) // per_page or 1
        if not logs: bot.answer_callback_query(call.id, "আপনার কোনো SMS পাঠানোর ইতিহাস নেই।", show_alert=True); return
        history_text = f"📜 **আপনার SMS History** (পেজ: {page}/{total_pages})\n\n"
        for log in logs: dt_obj = datetime.datetime.fromisoformat(log[1]); history_text += f"📞 নম্বর: `{log[0]}`\n🗓️ সময়: {dt_obj.strftime('%Y-%m-%d %H:%M')}\n---\n"
        row = []; keyboard = types.InlineKeyboardMarkup()
        ...
        keyboard.add(*row); keyboard.add(types.InlineKeyboardButton("🔙 মূল মেনু", callback_data="main_menu"))
    ...
    # --- অ্যাডমিন কলব্যাক ---
        ...
        try: bot.edit_message_text(stats_text, call.message.chat.id, call.message.message_id, reply_markup=keyboard, parse_mode="Markdown")
        except telebot.apihelper.ApiTelegramException as e:
            if "message is not modified" not in str(e): raise e
